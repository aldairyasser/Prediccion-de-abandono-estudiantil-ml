...
def pinta_distribucion_categoricas(df, columnas_categoricas, relativa=False, mostrar_valores=False): 
    # Te muestra la frecuencia de las columnas categoricas que le hayas pasado, 
...
def plot_categorical_relationship_fin(df, cat_col1, cat_col2, relative_freq=False, show_values=False, size_group = 5): 
    # La relación entre 2 columnas categoricas de un dataframe
    # Prepara los datos
...
def plot_grouped_histograms(df, cat_col, num_col, group_size): 
    # Categorico y numérica histograma Columnas y líneas de colores
    unique_cats = df[cat_col].unique()
    num_cats = len(unique_cats)
...
def card_tipo(df,umbral_categoria = 7, umbral_continua = 30):
